...
local nullTex = Graphics.loadImage(Misc.resolveFile("libs/blocks/null.png"))
    ...
    for _,block in Block.iterateIntersecting(c.x, c.y, c.x + c.width, c.y + c.height) do
        if block.id == fakeblocks.fakeblockID and not block.isHidden then
            ...
            if not maskShader then
                ...
                softCircleShader:compileFromFile(nil, Misc.resolveFile("libs/shaders/effects/softCircle.frag"))
                solidShader = Shader()
                solidShader:compileFromFile(nil, Misc.resolveFile("libs/shaders/effects/solidColor.frag"))
            end
