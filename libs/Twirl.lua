...
local twirlSfx = Misc.resolveFile("audio/sfx/twirl.flac")
...
--Animation
twirl.frames = {15, -2,-2,-2, 13,13,13, 2,2}
twirl.animSpeed = 2
local twirlAnim = playeranim.Anim(twirl.frames, twirl.animSpeed)
function animLength()
	local count = 0
	for dogssake in pairs(twirl.frames) do 
		count = count + 1
	end
	return count
end
...
function twirl.onTick() for k, p in ipairs(Player.get()) do
	...
	--Stop Animation
	if timeline[k] >= twirl.animSpeed*animLength() or timeline[k] == 0 or isOnMount() then
		twirlAnim:stop(p)
	end
