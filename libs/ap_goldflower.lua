...
apt.spritesheets = {
    Graphics.loadImage(Misc.resolveFile("mario-ap_goldflower.png")), --Mario
    ...
    if player.keys.run == KEYS_PRESSED or player.keys.altRun == KEYS_PRESSED then
        if player.keys.altRun == KEYS_PRESSED and (player.character == 3 or player.character == 4) then
            if not ducking() and player.mount == 0 then
                local v = NPC.spawn(982, player.x, player.y - 800, player.section)
            ...
            local v
            if not ducking() then
                v = NPC.spawn(982, player.x + 0.5 * player.width + 0.5 * player.width * player.direction, player.y + 16, player.section, false, true)
                v.speedY = 4
