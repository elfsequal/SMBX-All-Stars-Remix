...
local function init(v)
	...
	if not data.init then
		...
		data.platforms[1] = NPC.spawn(config.platformId, v.x + (v.width / 2 +5) - 40, v.y + 24 + v.ai2, v.section)
		data.platforms[2] = NPC.spawn(config.platformId, v.x + (v.width / 2 -5) + settings.len - 24, v.y + 24 + v.ai2, v.section)
